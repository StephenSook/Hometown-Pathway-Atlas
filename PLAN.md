...
| 3.12 | React Query wiring | `frontend/src/hooks/{useRegion,useAnalogs,usePathway}.ts` + `frontend/src/lib/queryClient.ts` | Stephen | 🟡 | 3.7, 2.6 | **Scaffolding 🟡 ready** (commits a3ebaa9 + 25da50f). 3 typed hooks against locked api.ts contracts, dependent-query chain wired, QueryClientProvider mounted in App.tsx, hackathon-tuned defaults (5min staleTime, retry: 1, no refetchOnWindowFocus). Day 4 swap is substitution-only at HomePage.tsx data-source sites — see 5-step integration comment in handleSubmit. **Marked ✅ once Vinh's task 2.6 endpoints are live + smoke verified end-to-end.** |

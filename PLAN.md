...
| 3.11 | ComplianceLog ★ + LogEntry | `frontend/src/components/{ComplianceLog,LogEntry}.tsx` | Stephen | ✅ | 3.6 | Day 6 AM. Pillar 4 demo moment. Fixed right sidebar (z-30) desktop, FAB-toggled bottom drawer mobile per §3 spec. Two columns RULES + GEMINI, both `role="log" aria-live="polite"`. demoMode prop forces canonical fail→fixed sequence; production guard limits to dev + localhost only (Cloud Run `*.run.app` excluded). Pass entries auto-collapse 1.5s. Fail→fixed dedupe by (layer, check) + stable key gives in-place crossfade via Framer color transitions. WCAG-AA fix: before-phrase uses dark text + amber strikethrough decoration. useReducedMotion honored. Cold-check round 1 caught 12 issues (production guard too permissive, key-defeats-crossfade, auto-collapse leaves empty, mobile drawer always-on, useEffect StrictMode race, aria-live placement, prefers-reduced-motion, amber contrast, stagger drop, aria-label/visible mismatch, DEMO_SCRIPT dedupe, entries default) — all fixed pre-commit. |

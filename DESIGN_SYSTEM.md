...
| `ComplianceLog` | Mobile: bottom drawer slide-up triggered by floating action button (NEVER fixed sidebar — eats mobile screen). Desktop (md+): fixed right sidebar 380px wide with collapse-to-chip + drag-anywhere overlay (added 2026-05-04). Default state expanded; user can Minus to collapse to a compact "AUDIT N/N" chip + drag the panel/chip anywhere on screen via the GripHorizontal handle. Reset position button surfaces conditionally when dragged. | `<aside className="fixed z-30 md:flex md:right-4 md:top-24 md:bottom-4 md:w-[380px]">` (expanded) or `<button className="hidden md:inline-flex fixed right-4 top-24 z-30">` (collapsed chip) |
